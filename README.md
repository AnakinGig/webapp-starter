...
- **Two-factor authentication (TOTP + backup codes)** - Settings → Security: enable by scanning a QR code (client-side rendered, secret never leaves the browser) with any authenticator app (Google Authenticator, 1Password, etc.), then confirm with a 6-digit code; 10 one-time backup codes are shown at setup and can be regenerated (old ones are invalidated); disabling and regenerating require re-entering your password (OAuth-only accounts skip it). Sign-in then asks for the code with an optional "trust this device for 30 days" checkbox and a backup-code fallback. Security emails are sent when 2FA is enabled or disabled. **Known limitation**: OAuth (social) sign-ins skip the challenge in better-auth 1.6 - only email/password sign-in triggers it; watch the issue tracker for plugin support
...
> No env vars are needed for 2FA - it works out of the box with the existing `BETTER_AUTH_SECRET` (used to encrypt the TOTP secret and backup codes at rest).
...
- [x] **Two-factor authentication (TOTP)** - Settings → Security: enable via QR + 6-digit confirm, 10 one-time backup codes (regenerable), disable/regenerate require password re-entry; sign-in shows an inline challenge step with trust-device (30d) and backup-code options; security emails on enable/disable. Server: `twoFactor` plugin on the auth instance (`convex/auth.ts`), client methods via `twoFactorClient()` in `src/lib/auth-client.ts`, UI in `src/components/settings/two-factor-section.tsx`. **Limitation**: OAuth sign-ins don't trigger the challenge (plugin only intercepts email/username/phone sign-in); per-challenge 5-attempt cap + rate limits apply; account-level lockout disabled because the Convex twoFactor table can't store its counters
- [ ] **Security notifications** - email the account owner when the password is changed or the account is deleted (a stolen-session safety net; reuse `convex/email.ts`). Already done: 2FA enable/disable notifications
