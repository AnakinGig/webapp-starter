...
- Email verification is enabled via the `emailVerification` option. Both `sendVerificationEmail` and `emailAndPassword.sendResetPassword` route through `sendActionEmail` in `src/lib/email.ts` (Resend; without `RESEND_API_KEY` the link is logged to the dev console and prod logs an error without the link - never log the token-bearing URL in prod). `user.sendVerificationEmail` is a self-service tRPC procedure with a 60s in-memory cooldown. Email/password signups start with `emailVerified: false`; OAuth signups are pre-verified. `RESEND_API_KEY` / `RESEND_EMAIL_FROM` are optional env vars (see `src/env.js` + `.env.example`); the default from-address is Resend's shared test domain `onboarding@resend.dev`.
- Password reset uses better-auth's built-in flow: `user.requestPasswordReset` (public tRPC procedure, 60s per-email in-memory cooldown, anti-enumeration - the response never reveals whether an email has an account) triggers `sendActionEmail`. The emailed link routes through `/api/auth/reset-password/:token` and lands on `/reset-password?token=...` (or `?error=INVALID_TOKEN`). `revokeSessionsOnPasswordReset: true` signs out all sessions on reset.
