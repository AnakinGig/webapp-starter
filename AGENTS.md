...
- Auth/email secrets (`BETTER_AUTH_SECRET`, `SITE_URL`, `BETTER_AUTH_<PROVIDER>_CLIENT_ID/SECRET`, `RESEND_API_KEY`, `RESEND_EMAIL_FROM`) live ON THE CONVEX DEPLOYMENT - set with `npx convex env set <KEY> <value>` (or the dashboard). They are NOT in `.env.example` values.
- **OAuth providers are env-driven**: `convex/oauth.ts` is the catalogue; setting `BETTER_AUTH_<PROVIDER>_CLIENT_ID` + `..._CLIENT_SECRET` (e.g. `BETTER_AUTH_GOOGLE_*`) enables that provider everywhere (login/register buttons via the public query in `convex/providers.ts`, and the Settings -> Account section). Unset a pair to remove the provider - no code changes. Do NOT add hardcoded `socialProviders` entries in `convex/auth.ts`; extend `OAUTH_PROVIDERS` in `convex/oauth.ts` instead.
- `SKIP_ENV_VALIDATION=1` bypasses Next env validation (useful for Docker builds).

## Auth (better-auth on Convex)

- better-auth runs INSIDE Convex: `convex/auth.ts` holds all options (password policy, email hooks, rate limiting, account linking, env-driven OAuth via `convex/oauth.ts`, the custom `role` field). Next.js just proxies `/api/auth/*` to the deployment (`src/app/api/auth/[...all]/route.ts` → `convex/http.ts`).
- Account linking is enabled (`account.accountLinking` in `convex/auth.ts`): signed-in users can connect/disconnect OAuth providers from Settings -> Account via `/api/auth/link-social`, `/api/auth/unlink-account`, `/api/auth/list-accounts`. Secure defaults are kept: provider email must match the account email (`allowDifferentEmails` false), the last sign-in method can't be unlinked (`allowUnlinkingAll` false), and implicit linking on sign-in only happens for configured providers when the local email is verified (`requireLocalEmailVerified`). OAuth callback failures land on `/error?error=...` (see `src/app/error/page.tsx`).
...
- Email flows: `sendVerificationEmail` + `sendResetPassword` hooks call `sendActionEmail` in `convex/email.ts` (Resend; without `RESEND_API_KEY` the link is logged to the function logs in dev - never log the token-bearing URL in prod). Dev vs prod is decided by the `ENVIRONMENT` Convex env var (`"production"` = prod), NOT `NODE_ENV` - Convex sets `NODE_ENV` to `"production"` on every deployment, even dev ones. The resend button and forgot-password form call the proxied routes directly (`/api/auth/send-verification-email`, `authClient.forgetPassword`) - both are rate-limited server-side by better-auth (see `rateLimit` in `auth.ts`).
